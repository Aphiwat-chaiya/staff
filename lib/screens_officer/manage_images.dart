...
class _ImageManagementPageState extends State<ImageManagementPage> {
  ...
  Future<void> _fetchImages() async {
    ...
    var response = await http.get(Uri.parse('http://10.0.2.2:3000/get_images'));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        _activeImages = images
            ...
            .map((img) => {
                  ...img,
                  'image_url': 'http://10.0.2.2:3000/uploadnews/${img['image_url']}',
                })
            .toList();
        _allImages = images
  ...
  Future<void> _updateImageStatus(int imageId, String newStatus) async {
    var response = await http.post(
      Uri.parse('http://10.0.2.2:3000/update_image_status'),
